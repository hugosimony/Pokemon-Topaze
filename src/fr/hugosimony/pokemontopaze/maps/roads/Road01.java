...
public class Road01 extends JPanel {
	...
	public Road01(Game game, int locationX, int locationY, Direction direction, int mapLocationX, int mapLocationY) {
		...
		setHerbs();
		setAnimations();
		toIntertown01 = new IntTuple(4064, 1834);
		toIntertown02 = new IntTuple(3232, 778);
	...
	public void setAnimations() {
		...
		if(Variables.BATTLERS.contains("campinggirl001")){
			animationTiles.add(new IntTriple(3584, 1322, 3));
			...
			if(IntTriple.containsTuple(animationTiles, finalAnimation)) {
				...
				if(animation == 0)
					...
					if(game.deplacement.getPosition().x == 3648)
						new Timer().scheduleAtFixedRate(game.deplacement.new MoveDirection(Direction.RIGHT, false, 7, false, true, new IntTuple(3680, game.deplacement.getPosition().y), 
								"r", Direction.RIGHT, "Je devrais aller récupérer un Pokémon.", true), 0, 8);
					else 
						new Timer().scheduleAtFixedRate(game.deplacement.new MoveDirection(Direction.UP, false, 7, false, true, new IntTuple(game.deplacement.getPosition().x, 1770), 
